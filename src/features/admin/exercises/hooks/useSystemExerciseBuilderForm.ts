...
import { SystemExerciseInputSchema } from '@/features/admin/exercises/api/contracts';
import { builderMetadataQueryOptions } from '@/features/exercises/builder/api/queries';
import {
...
const emptyScalars: ExerciseScalars = {
  ...emptyExerciseScalars,
  isPublic: false,
};
...
type UseSystemExerciseBuilderFormOptions = {
  exerciseId?: string;
};
...
export function useSystemExerciseBuilderForm({ exerciseId }: UseSystemExerciseBuilderFormOptions) {
  ...
  const [scalars, setScalars] = useState<ExerciseScalars>(emptyScalars);
  ...
  const { mutate: mutateCreate, isPending: isCreating } = useMutation({
    mutationFn: createSystemExercise,
    onSuccess: (response) => onSaveSuccess(response.id),
  });

  const { mutate: mutateUpdate, isPending: isUpdating } = useMutation({
    mutationFn: (payload: Parameters<typeof updateSystemExercise>[1]) => updateSystemExercise(exerciseId!, payload),
    onSuccess: (response) => onSaveSuccess(response.id),
  });

  const { mutate: mutateDelete, isPending: isDeleting } = useMutation({
    mutationFn: () => deleteSystemExercise(exerciseId!),
    onSuccess: () => {
      releaseGuard();
  ...
  const submit = useCallback((): SystemExerciseBuilderSubmitResult => {
    const snapshot = getSnapshot();
    const parsed = SystemExerciseInputSchema.safeParse(toSystemExerciseInput(snapshot));
