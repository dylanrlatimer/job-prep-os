...
Leftover: `TopicBuilderPage` still inlines the same `Field` instead of importing `common/components/Field.tsx`.
...
```
Appears in `PracticePage`, `ExercisePracticePage`, `QuestionDetailPage`, `ExerciseDetailPage`, `BrowseQuestionDetailPage`, `BrowseExerciseDetailPage` (and the two admin system view pages). Extract to `common/components/SourceCitation.tsx`.

## 6. Back-nav link — still ~10 identical copies
```tsx
...
```
Appears in every practice/detail/builder/browse-detail/admin-view page. Extract to `common/components/BackLink.tsx` — props: `href`, `label`.

## 7. Page load error state — component exists, copies remain
`common/components/PageLoadError.tsx` exists and the six list pages use it. Still inlined in builders, practice, detail, browse-detail, and topic builder (~8 remaining).

## 8. `hasAttempts` and `resultLabelKey`
~~`hasAttempts` (checks `incorrect + partial + correct > 0`) is copy-pasted into `TheoryRepositoryPage`, `ExerciseRepositoryPage`, `PracticePage`, `ExercisePracticePage`, `QuestionDetailPage`, `ExerciseDetailPage`.~~ Now in `features/theory/lib/attempt-result-styles.ts`.

`resultLabelKey` (maps result → i18n key suffix) is still copied in `PracticePage`, `ExercisePracticePage`, `QuestionDetailPage`, `ExerciseDetailPage`.
...
Done via optional `emptyLabel` on the shared component.
...
Shared chrome/helpers (`ListPageLayout`, `list-filters`, skeleton, `PageLoadError`). Twin pages and local rows kept on purpose.
...
Same shared chrome as #10. `AdminGate` + local rows kept.
...
Now `mapZodFieldErrorsFlat` / `mapZodFieldErrorsNested`.
...
Collapsed via `apiLayer` injection on the shared builder hooks. No generic `useBuilderForm`.

---

**Still open:** #5, #6, leftover of #1 (`TopicBuilderPage` Field), leftover of #7 (non-list pages), leftover of #8 (`resultLabelKey`), #13.
